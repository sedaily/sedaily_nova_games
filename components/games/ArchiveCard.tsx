...
export function ArchiveCard({ gameType, date, questionCount, isToday, href }: ArchiveCardProps) {
  ...
  return (
    <Link
      href={href}
      className={`group block rounded-2xl border ${config.borderOpacity} ${config.listBg} backdrop-blur-[2px] ${config.shadow} ${config.hoverShadow} hover:-translate-y-0.5 transition-all focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2`}
      style={{ outlineColor: config.focusColor }}
    >
      <div className="flex items-center gap-4 md:gap-5 p-6 md:p-7 min-h-[140px] md:min-h-[160px]">
        <div
          className="flex-shrink-0 relative w-[80px] h-[120px] md:w-[100px] md:h-[150px] rounded-[14px] overflow-hidden border border-black/25 shadow-sm woodcut-texture"
